...
/**
 * 读取 midlife_awakening_assessments.claim_code（中场觉醒力测评）。
 * 新插入由触发器生成；历史/缺失会调用 RPC 补写。
 */
export function useMidlifeAwakeningClaimCode(assessmentId?: string | null) {
    ...
    (async () => {
      ...
      try {
        ...
        const { data: gen, error: genErr } = await sb.rpc(
          "generate_midlife_awakening_claim_code"
        );
        ...
        console.warn("[useMidlifeAwakeningClaimCode] failed", e);
      } finally {
