...
export default function Packages() {
  ...
  // 构建有劲产品（会员套餐 + 有劲训练营入口）
  const youjinPackages = [
    ...basePackages.filter(pkg => pkg.category === 'youjin'),
    {
      id: 'youjin-camps',
      name: '有劲训练营',
      duration: '免费',
  ...
  // 构建绽放产品（训练营 + 合伙人）
  const bloomCamps = campTemplates
    .filter(t => t.category === 'bloom')
    .map(camp => ({
      id: `camp-${camp.id}`,
      name: camp.camp_name,
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'youjin' | 'bloom')} className="w-full">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 h-14">
              {productCategories.map((category) => (
                <TabsTrigger 
                  key={category.id} 
                  value={category.id}
                  className="text-base gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary/20 data-[state=active]:to-primary/10"
                >
            ...
            <TabsContent value={activeTab} className="mt-8 space-y-8">
              ...
              <div className="text-center space-y-2 max-w-2xl mx-auto">
                <h2 className={`text-2xl font-bold bg-gradient-to-r ${currentCategory?.gradient} bg-clip-text text-transparent`}>
                        ...
                        <div className="space-y-1">
                          {pkg.isCampEntry ? (
                            ...
                            <>
                              ...
                              {'totalValue' in pkg && pkg.totalValue > 0 && (
