...
import { Heart, Compass, Users, Sun, Moon, PenLine, Calendar, TrendingDown, Brain, BookOpen, Lightbulb, Target, Zap, Sparkles, BarChart as BarChartIcon, MessageCircle, Activity, Wallet } from "lucide-react";
...
const Introduction = () => {
  ...
  // 痛点数据
  const painPoints = ["压力很大，却说不清到底在烦什么。", "脑子里很吵，睡前总是停不下来。", "常常重复同样的情绪与关系冲突。", "明明想改变，却找不到开始的方法。"];
  ...
  const aiSteps = [{
    ...
    step: 2,
    title: "AI 命名情绪",
    desc: "帮你辨识焦虑？失落？恐惧？愤怒？",
    icon: Brain
  }, {
    step: 3,
    title: "找出原因与需求",
    desc: "AI 会问对问题，帮你理解心里真正想表达的。",
    icon: Lightbulb
  }, {
    step: 4,
    title: "给行动建议与今日洞察",
    desc: "当下做得到的小行动，让情绪真的被转化。",
    icon: Sparkles
  }];
  ...
  const dailyRoutine = [{
    ...
    time: "白天 · 2–3 分钟",
    task: "任何情绪，用一句话写成情绪日记。",
    icon: PenLine
  }, {
    time: "晚上 · 6 分钟",
    task: "AI 帮你做复盘，并生成《有劲日报》。",
    icon: Moon
  }, {
    time: "每周一次",
    task: "自动生成《有劲周报》，看到自己的趋势。",
    icon: Calendar
  }];
  ...
  const features = [{
    ...
    examples: ["说出一句感受，AI 会帮你命名情绪（焦虑、失落、愤怒等）", "通过追问找到情绪背后的真正原因", "基于你的需求，给出当下可执行的小行动", "自动生成情绪洞察，帮助你理解自己的模式"]
  }, {
    icon: "📘",
    title: "有劲日报 / 有劲周报",
    desc: "自动整理：洞察、行动、趋势、故事。",
    details: "AI 自动为你生成结构化的成长报告：",
    examples: ["每日洞察：总结当天的情绪主题和收获", "行动清单：回顾你完成的行动和未完成的计划", "情绪趋势：可视化展示你的情绪变化曲线", "成长故事：用叙事方式记录你的改变历程"]
  }, {
    icon: "🌈",
    title: "每日能量宣言 + 宣言卡",
    desc: "用一句话为你定调今天。",
    details: "每天早晨收到专属的能量宣言：",
    examples: ["基于你的情绪历史，AI 生成个性化宣言", "可生成精美的宣言卡片，设为手机壁纸", "帮助你用积极的心态开启新的一天", "建立正向的心理暗示和自我认同"]
  }, {
    icon: "⚡",
    title: "高能量测评",
    desc: "测你在「共振 / 觉醒 / 升维」哪一个阶段。",
    details: "科学评估你的成长阶段：",
    examples: ["共振期：开始看见自己的情绪和模式", "觉醒期：理解模式背后的原因和需求", "升维期：能够主动改变行为和思维方式", "根据测评结果，AI 会调整对话策略和建议"]
  }, {
    icon: "💰",
    title: "财富卡点测评",
    desc: "找到你与金钱关系的卡点。",
    details: "深入探索你的金钱信念和模式：",
    examples: ["识别你对金钱的核心信念（匮乏、恐惧、不配得等）", "找到影响财富的情绪卡点", "理解金钱背后的深层需求", "获得调整金钱关系的具体建议"]
  }, {
    icon: "💬",
    title: "AI 生活 Q&A",
    desc: "情绪、关系、职场、家庭、决策都能问。",
    details: "随时随地的智能生活顾问：",
    examples: ["情绪困扰：如何处理焦虑、愤怒、失落", "关系问题：亲密关系、职场关系、家庭关系", "职场决策：工作选择、职业发展、团队协作", "个人成长：目标设定、习惯养成、自我认知"]
  }, {
    icon: "🌱",
    title: "社群共振、绽放故事、训练营",
    desc: "不再独自改变，一群人一起成长。",
    details: "加入温暖的成长社群：",
    examples: ["21天情绪日记训练营：系统化的成长路径", "绽放故事：分享你的改变故事，激励他人", "社群打卡：与同行者互相支持和见证", "小组陪伴：找到志同道合的成长伙伴"]
  }];
  ...
  // 每日任务
  const dailyTasks = ["今日能量宣言", "情绪日记", "晚间复盘 / 日报", "每周周报", "社群打卡（可选）"];

  // 快捷指令
  const quickCommands = ["开始情绪日记", "生成今日能量宣言", "生成宣言卡", "开始高能量测评", "财富测评", "生成今日有劲日报", "生成本周周报", "我需要建议", "我想问一个问题"];
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 pt-3 relative z-20">
          <div className="flex items-center justify-between">
            ...
            <IntroShareDialog config={introShareConfigs.introduction} />
          </div>
        </div>
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            ...
            return <Card key={item.step} className="hover:shadow-xl transition-shadow duration-300">
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => <Dialog key={index}>
                ...
                <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                  ...
                  <div className="mt-6 space-y-4">
                    ...
                    <ul className="space-y-3">
                      {feature.examples.map((example, exampleIndex) => <li key={exampleIndex} className="flex items-start gap-3 p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
      ...
      <section id="camp" className="container mx-auto px-4 py-16 bg-gradient-to-b from-accent/10 to-background">
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="mb-8 hover:shadow-xl transition-shadow">
            ...
            <CardContent>
              <div className="space-y-4">
                {campPhases.map((phase, index) => <div key={index} className={`p-5 rounded-lg ${phase.color} border-2 border-primary/10`}>
          ...
          <Card className="mb-8 hover:shadow-xl transition-shadow">
            ...
            <CardContent>
              <ul className="space-y-3">
                {dailyTasks.map((task, index) => <li key={index} className="flex items-center gap-3">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button size="lg" variant="outline" onClick={handleGetStarted} className="hover:scale-105 transition-transform gap-2">
              📘 立即体验
            </Button>
          </div>
        </div>
      ...
      <section className="container mx-auto px-4 py-16 md:py-20 bg-gradient-to-br from-background via-background to-accent/5">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-8 md:space-y-12">
            {faqCategories.map((category, catIndex) => <div key={catIndex} className="space-y-4 md:space-y-6">
                {/* 分类标题区域 */}
                ...
                return <AccordionItem key={itemIndex} value={`${catIndex}-${itemIndex}`} className="border rounded-lg px-3 md:px-4 lg:px-6 bg-card hover:bg-accent/5 hover:border-primary/30 transition-all duration-200">
                        <AccordionTrigger className="text-left hover:no-underline py-3 md:py-4 lg:py-5 group">
                          <div className="flex items-start gap-2 md:gap-3 w-full">
                            <Badge variant="outline" className="shrink-0 mt-0.5 min-w-[28px] md:min-w-[32px] h-6 md:h-7 rounded-full flex items-center justify-center text-xs md:text-sm font-semibold group-hover:bg-primary/10 group-hover:border-primary/50 transition-colors">
                              {questionNumber}
                            </Badge>
                ...
                {/* 分类分隔线 */}
                {catIndex < faqCategories.length - 1 && <Separator className="my-6 md:my-8" />}
              </div>)}
          </div>
        </div>
      </section>
