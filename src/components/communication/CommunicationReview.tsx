...
export const CommunicationReview = () => {
  ...
  const generateReview = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('generate-communication-review', {
        body: {
          period: reviewType,
      ...
      if (data.error) {
        if (data.error === 'no_data') {
          toast({
            title: "暂无数据 📭",
            description: data.message || "该时间段内没有沟通记录",
          });
          return;
        }
      ...
      toast({
        title: "复盘报告已生成 ✨",
        description: `基于 ${data.briefingsCount} 条沟通记录`,
      });
      ...
      <Card className="p-4 md:p-6 space-y-4 md:space-y-6">
        ...
        <div className="space-y-3 md:space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 md:gap-0 md:grid-cols-1 md:space-y-0">
                <div className="flex items-center space-x-2 md:space-x-3 p-2 md:p-3 rounded-lg hover:bg-accent/50 transition-colors border md:border-0 border-border">
                  ...
                  <Label htmlFor="weekly" className="cursor-pointer flex-1 text-xs md:text-sm leading-tight">
                    周报
                    <span className="hidden md:inline">（每周沟通总结）</span>
                  </Label>
                </div>
                <div className="flex items-center space-x-2 md:space-x-3 p-2 md:p-3 rounded-lg hover:bg-accent/50 transition-colors border md:border-0 border-border">
                  ...
                  <Label htmlFor="monthly" className="cursor-pointer flex-1 text-xs md:text-sm leading-tight">
                    月报
                    <span className="hidden md:inline">（每月沟通复盘）</span>
                  </Label>
                </div>
              </div>
            </RadioGroup>
          ...
          <Button
            ...
            className="w-full text-sm md:text-base bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600"
            size="lg"
          >
            {isGenerating ? (
              <>
