...
import { Pin, PinOff, Trash2, RefreshCw, MessageSquare, TrendingUp, Heart, ThumbsUp, MessageCircle } from "lucide-react";
...
export default function CommunityPostsManagement() {
  ...
  const { data: stats, isLoading: loadingStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { total: totalRes.count || 0, today: todayRes.count || 0, pinned: pinnedRes.count || 0, interactions };
    },
  });
  ...
  const togglePin = async (post: CommunityPost) => {
    ...
    const { error } = await supabase
      .from("community_posts")
      .update({ is_pinned: newPinned, pinned_at: newPinned ? new Date().toISOString() : null })
      .eq("id", post.id);
    if (error) {
  ...
  return (
    <AdminPageLayout
      title="社区动态管理"
      description="管理社区帖子，包括置顶、删除和内容审核"
      actions={
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <AdminStatCard label="总帖数" value={stats?.total || 0} icon={MessageSquare} loading={loadingStats} />
        <AdminStatCard label="今日新增" value={stats?.today || 0} icon={TrendingUp} accent="bg-green-500/10 text-green-600" loading={loadingStats} />
        <AdminStatCard label="置顶帖" value={stats?.pinned || 0} icon={Pin} accent="bg-amber-500/10 text-amber-600" loading={loadingStats} />
        <AdminStatCard label="总互动" value={stats?.interactions || 0} icon={Heart} accent="bg-rose-500/10 text-rose-600" loading={loadingStats} />
      </div>
      ...
      {/* Table */}
      <AdminTableContainer minWidth={800}>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/40">
              ...
              <th className="text-center font-medium text-muted-foreground px-2 py-3 w-[6%]"><ThumbsUp className="h-3.5 w-3.5 mx-auto" /></th>
              <th className="text-center font-medium text-muted-foreground px-2 py-3 w-[6%]"><MessageCircle className="h-3.5 w-3.5 mx-auto" /></th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={8} className="text-center py-12 text-muted-foreground">{loading ? "加载中…" : "暂无数据"}</td></tr>
            ) : filtered.map(post => (
              <tr key={post.id} className={`border-b last:border-0 transition-colors hover:bg-muted/30 ${post.is_pinned ? "bg-primary/[0.03]" : ""}`}>
                ...
                <td className="px-3 py-3 text-muted-foreground whitespace-nowrap">
                  {post.is_anonymous ? <span className="italic">匿名</span> : <span className="truncate block max-w-[80px]">{profiles[post.user_id] || post.user_id.slice(0, 6)}</span>}
                </td>
                <td className="px-3 py-3"><Badge variant="secondary" className="text-xs font-normal">{typeLabel[post.post_type] || post.post_type}</Badge></td>
                ...
                <td className="px-3 py-3 text-muted-foreground whitespace-nowrap text-xs">{format(new Date(post.created_at), "MM-dd HH:mm")}</td>
                <td className="px-3 py-3 text-center"><Badge variant={visibilityLabel[post.visibility]?.variant || "outline"} className="text-xs font-normal">{visibilityLabel[post.visibility]?.label || post.visibility}</Badge></td>
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-0.5">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => togglePin(post)} title={post.is_pinned ? "取消置顶" : "置顶"}>
                      {post.is_pinned ? <PinOff className="h-4 w-4 text-primary" /> : <Pin className="h-4 w-4" />}
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => setDeleteTarget(post)}>
