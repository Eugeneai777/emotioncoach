...
import {
  ArrowLeft, Users, ClipboardList, TrendingUp, RefreshCw,
  Search, Download, Phone, Copy, Eye, CalendarDays, BadgeCheck,
} from "lucide-react";
...
import { useAdminEmotionHealthInsights } from "@/hooks/useAdminEmotionHealthInsights";
...
function toCsv(rows: (RespondentRow & { isPaid: boolean })[]) {
  const header = ["昵称", "手机号", "国家码", "主导类型", "精力指数", "领取码", "已付费", "测评时间", "管理员备注", "标签"];
...
export default function EmotionHealthInsightsDetail() {
  const navigate = useNavigate();
  const { data, isLoading } = useAdminEmotionHealthInsights();

  const [search, setSearch] = useState("");
  const [patternFilter, setPatternFilter] = useState<string>("all");
  const [drawerRow, setDrawerRow] = useState<(RespondentRow & { isPaid: boolean }) | null>(null);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `情绪健康测评名单_${format(new Date(), "yyyyMMdd_HHmm")}.csv`;
    a.click();
  ...
  if (isLoading || !data) {
    return (
      <AdminPageLayout title="情绪健康测评 · 数据洞察" description="加载中...">
    ...
    <AdminPageLayout
      title={
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="ml-2 border-emerald-400 text-emerald-700 dark:text-emerald-300">内置</Badge>
        </div>
      }
      description="情绪健康答卷统计、用户画像与可执行运营名单(35+ 女性投放)"
    >
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <AdminStatCard label="付费转化" value={data.paidCount} icon={BadgeCheck} accent="bg-rose-500/10 text-rose-600" />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 mt-4">
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">阻塞维度分布</CardTitle></CardHeader>
          <CardContent className="h-64">
            {data.blockedDimensionDistribution.length === 0 ? (
              <div className="h-full flex items-center justify-center text-sm text-muted-foreground">暂无数据</div>
            ) : (
              <ResponsiveContainer>
                <BarChart data={data.blockedDimensionDistribution}>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">精力指数分布</CardTitle></CardHeader>
          <CardContent className="h-64">
            <ResponsiveContainer>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-muted-foreground">
                <tr>
                  ...
                  <th className="text-right px-4 py-2 font-medium">精力指数</th>
                  <th className="text-left px-4 py-2 font-medium">领取码</th>
                  <th className="text-left px-4 py-2 font-medium">付费</th>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-10 text-muted-foreground">暂无数据</td></tr>
                ) : (
                  filtered.map((r) => (
                    <tr key={r.resultId} className="border-t hover:bg-muted/20">
                            ...
                            {(r.adminNote || (r.adminTags && r.adminTags.length > 0)) ? (
                              <div className="flex items-center gap-1 mt-0.5 max-w-[200px]">
                                {r.adminTags?.slice(0, 2).map((t) => (
                                  <Badge key={t} variant="outline" className="text-[10px] px-1 py-0 h-4 border-amber-300 text-amber-700">{t}</Badge>
                                ))}
                                {r.adminNote && (
                                  <span className="text-[11px] text-muted-foreground truncate">📝 {r.adminNote}</span>
                                )}
                              </div>
                            ) : (
                              <button
                      ...
                      <td className="px-4 py-2">
                        {r.isPaid ? (
                          <Badge className="bg-rose-100 text-rose-700 hover:bg-rose-100 dark:bg-rose-950 dark:text-rose-300">已付费</Badge>
                        ) : (
                          <span className="text-xs text-muted-foreground">—</span>
                        )}
                      </td>
