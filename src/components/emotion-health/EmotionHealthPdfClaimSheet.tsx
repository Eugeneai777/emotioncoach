...
import EmotionHealthPdfClaimCard from "./EmotionHealthPdfClaimCard";
...
export function EmotionHealthPdfClaimSheet({
  open,
  onOpenChange,
  claimCode,
  loadingCode,
  ...
  useEffect(() => {
    ...
    const cacheKey = `${claimCode}::${battery}`;
    ...
    (async () => {
      ...
      try {
        ...
        console.error("[EH-ClaimSheet] preview gen failed", e);
      } finally {
        if (!cancelled) setGenerating(false);
      }
    })();
    return () => { cancelled = true; };
  }, [open, claimCode, avatarUrl, battery]);
  ...
  const handleDownloadImage = async () => {
    ...
    a.href = previewUrl;
    a.download = `情绪健康专属凭证_${claimCode || "code"}.png`;
        ...
        <div className="px-5 pb-6 space-y-4">
          <ol className="space-y-2 text-sm text-foreground/85 bg-rose-50/60 dark:bg-rose-950/20 rounded-xl p-3 border border-rose-100 dark:border-rose-900">
            ...
            <li className="flex gap-2">
              ...
              <span>把「截图 + 领取码」发给助教，<b>24 小时内</b>收到完整 PDF 报告 + 1 次 1v1 解读</span>
            </li>
          </ol>
...
function ClaimSkeleton({ claimCode, loading }: { claimCode?: string | null; loading: boolean }) {
  return (
    ...
    >
      <div className="text-pink-600 text-xs tracking-widest mb-2 font-semibold">🌸 「她」能量报告</div>
