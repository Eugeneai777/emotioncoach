  ...
  try {
    ...
    if (tradeStatus === 'TRADE_SUCCESS' || tradeStatus === 'TRADE_FINISHED') {
      ...
      if (order.status === 'paid') {
        ...
        const healCamps = order.user_id ? bundleCampMapHeal[order.package_key] : null;
        if (healCamps) {
          for (const healCamp of healCamps) {
            try {
              ...
              if (!existingCamp) {
                ...
                console.log(`[AlipayCallback] Repaired missing ${healCamp.campType} for ${order.package_key}:`, order.user_id);
              }
            } catch (repairErr) {
              console.error(`[AlipayCallback] ${healCamp.campType} camp repair error:`, repairErr);
            }
          }
        }
      ...
      // 跳过充值套餐（basic/standard_49/premium_99/member365），点数包允许重复购买
      ...
      const isRechargePackage = order.package_key && RECHARGE_ALLOWED_KEYS.has(order.package_key);
      if (order.user_id && order.package_key && !isStoreProduct && !isRechargePackage) {
        ...
        if (existingPaidOrder) {
          console.log(`[AlipayCallback] DUPLICATE PAYMENT DETECTED! Order ${orderNo} is duplicate of ${existingPaidOrder.order_no}. Marking as duplicate_paid.`);
          await supabase
            .from('orders')
            .update({
        ...
        if (bundleCampsNew) {
          for (const camp of bundleCampsNew) {
            try {
              ...
              console.log(`[AlipayCallback] ${order.package_key} camp purchase recorded for ${camp.campType}`);
            } catch (e) {
              console.error(`[AlipayCallback] ${camp.campType} camp purchase exception:`, e);
            }
          }
        }
      }
