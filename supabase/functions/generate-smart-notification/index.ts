...
请确保返回纯JSON格式，不要包含任何markdown标记。`;
    ...
    // 如果是预览模式或仅返回模式（如宝妈每日寄语），不保存到数据库，直接返回
    if (isPreview || context?.return_only) {
      return new Response(JSON.stringify({ 
        success: true,
        notification: {
          ...
          priority: priority,
          style: notificationData.style // 宝妈寄语风格标签
        }
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
