...
serve(async (req) => {
  ...
  try {
    ...
    // Query last 7 days of mood logs
    ...
    const aiResp = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `你是一个温和的亲子关系专家。请根据孩子的情绪记录，生成一段简短的周度情绪趋势摘要（2-3句话）。
要求：
- 语气温和、积极
- 只描述趋势，不暴露任何对话内容
- 如果有焦虑/低落情绪，用建设性的方式提及
- 最后给家长一个小建议
- 控制在80字以内`
          },
          {
            role: 'user',
            content: `以下是孩子近7天的情绪记录（仅标签和强度，无对话内容）：\n${moodSummary}\n\n请生成周度情绪摘要。`
          }
        ],
      }),
    });
    ...
    if (!aiResp.ok) {
      ...
      return new Response(
        JSON.stringify({
          summary: `本周孩子共使用${logs.length}次，主要情绪为「${topMood[0]}」。继续关注TA的状态吧 💜`,
          hasData: true,
    ...
    console.error('xiaojin-mood-summary error:', error);
    return new Response(
