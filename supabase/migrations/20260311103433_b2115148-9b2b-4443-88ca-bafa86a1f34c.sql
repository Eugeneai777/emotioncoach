...
CREATE OR REPLACE FUNCTION public.get_zhile_orders()
 RETURNS TABLE(id uuid, order_no text, user_id uuid, amount numeric, status text, buyer_name text, buyer_phone text, buyer_address text, shipping_status text, shipping_note text, pay_type text, paid_at timestamp with time zone, created_at timestamp with time zone, id_card_name text, id_card_number text, source text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  -- Branch 1: direct orders with known zhile package keys
  SELECT 
    o.id,
    o.order_no,
    o.user_id,
  ...
  -- Branch 2: store_orders with product name containing 知乐
  SELECT
    so.id,
    so.order_no,
  ...
  -- Branch 3: orders from health store with store_product_ prefix for 知乐 products
  SELECT
    o.id,
    o.order_no,
    o.user_id,
